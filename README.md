...
> 현재 상태: **Phase 7 (저장 / 메타 진행 / 온보딩)**. 10스테이지 50웨이브,
> 합성·전투·업그레이드 드래프트에 더해 진행도 저장, 스테이지 선택, 영구 업그레이드
> 상점, 최초 플레이 온보딩까지 동작합니다. 새로고침해도 진행도가 유지됩니다.
> 오디오만 아직 없어서 무음으로 동작합니다 — 파일을 넣는 순간 코드 수정 없이
